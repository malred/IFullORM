...
import org.malred.utils.Operate;
...
public class t {
    ...
    public void testSelectBuild() {
        String sql = SqlBuilder.build().select("tb_user")
        ...
        String sql_cols_option2 = SqlBuilder.build()
                .select("tb_user", "username", "gender", "addr")
                .where("id", SqlCompareIdentity.NE)
                .where("password", SqlCompareIdentity.NE)
                .sql();
        System.out.println(sql_cols_option2);

        String sql_count = SqlBuilder.build()
                .count("tb_user")
                .where("id", SqlCompareIdentity.GT)
                .sql();
    ...
    public void testSelectBuildTbName() {
        ...
        String sql_cols1 = SqlBuilder.build()
                .tbName("tb_user")
                .select(new String[]{"username", "gender", "addr"})
                .where("id", SqlCompareIdentity.NE)
                .sql();
        System.out.println(sql_cols1);

        String sql_cols_option21 = SqlBuilder.build()
                .tbName("tb_user")
                .select(new String[]{"username", "gender", "addr"})
                .where("id", SqlCompareIdentity.NE)
                .where("password", SqlCompareIdentity.NE)
        ...
        String sql_count1 = SqlBuilder.build()
                .tbName("tb_user")
                .count()
                .where("id", SqlCompareIdentity.GT)
                .sql();
    ...
    public void testSelectMulti() throws Exception {
        ...
        String sql_cols_option2 = SqlBuilder.build()
                .select("tb_user", "username", "gender", "addr")
                .where("id", SqlCompareIdentity.NE)
                .where("gender", SqlCompareIdentity.NE)
                .sql();
    ...
    public void testInsertBuildTbName() {
        String sql = SqlBuilder.build()
                .tbName("tb_user")
                .insert(new String[]{"username", "password", "addr", "gender"})
                .sql();
        System.out.println(sql);
    }
    ...
    public void testUpdateProxy() {
        ...
        cnt = mapper.update(
                new TbUser(211, "eman", "ssap", null, null));
        System.out.println("影响了" + cnt + "条数据");

        cnt = mapper.update(
                new TbUser(211, "name", "pass", null, null));
        System.out.println("影响了" + cnt + "条数据");
    }
    ...
    public void testInsertProxy() {
        ...
        cnt = mapper.insert(
                new TbUser(0, "eman", "ssap", null, null));
        System.out.println("影响了" + cnt + "条数据");
    }
    ...
    public void testSelectAnnotation() {
        ...
        // todo: 根据接口方法的返回值获取泛型类型,动态判断返回类型
        UserRepository puMapper =
                Operate.getMapper(UserRepository.class, ProductAndUser.class);
        List<ProductAndUser> userAndProductJoin = puMapper.findUserAndProductJoin(7);
    ...
    public void testDeleteAnnotation() {
        UserRepository mapper =
                Operate.getMapper(UserRepository.class, TbUser.class);
        int cnt = mapper.delUser(211, "pass");
        System.out.println("影响了" + cnt + "条数据");
    }
